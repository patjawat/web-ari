...
class MediaBridge extends Component {
  ...
  componentDidMount() {
    ...
    this.props.socket.on('visit',(data) => {
      console.log(data)
      this.setState({visit:data[0]})
    });
  }
    ...
    return (
      <div className={`media-bridge ${this.state.bridge}`}>
       <div style={{
          ...
          borderRadius:'10px',
          padding:'10px',
          backgroundColor:'#24242575',
          width:'500px'}}>
      <h5 className="card-title">แพทย์ : {this.state.visit.doctor}</h5>
  


  <div className="card">
    <div className="card-body">
      <h5 className="card-title">ผู้ป่วย : {this.state.visit.fullname}</h5>
      <p className="card-text">HN : {this.state.visit.hn} </p>
      <p className="card-text">VN : {this.state.visit.vn} </p>
      <p className="card-text">HN : {this.state.visit.fullname} </p>
    </div>
  </div>
